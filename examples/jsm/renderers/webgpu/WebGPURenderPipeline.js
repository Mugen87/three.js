...
class WebGPURenderPipeline {
	...
	_getArrayStride( type, bytesPerElement ) {
		...
		if ( type === 'float' ) return bytesPerElement;
		if ( type === 'vec2' ) return bytesPerElement * 2;
		if ( type === 'vec3' ) return bytesPerElement * 3;
		if ( type === 'vec4' ) return bytesPerElement * 4;

		if ( type === 'int' ) return bytesPerElement;
		if ( type === 'ivec2' ) return bytesPerElement * 2;
		if ( type === 'ivec3' ) return bytesPerElement * 3;
		if ( type === 'ivec4' ) return bytesPerElement * 4;

		if ( type === 'uint' ) return bytesPerElement;
		if ( type === 'uvec2' ) return bytesPerElement * 2;
		if ( type === 'uvec3' ) return bytesPerElement * 3;
		if ( type === 'uvec4' ) return bytesPerElement * 4;
	...
	_parseShaderAttributes( shader, geometry ) {
		...
		while ( shaderAttribute = regex.exec( shader ) ) {
			...
			const geometryAttribute = geometry.getAttribute( name );
			const bytesPerElement = ( geometry !== undefined ) ? geometryAttribute.array.constructor.BYTES_PER_ELEMENT : 4;
